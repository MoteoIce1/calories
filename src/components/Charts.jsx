import { movingAverage } from '../utils/stats.js';
          ...
          <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
            {yTicks.map((t, k) => (
              <g key={k}>
                ...
                <text x={padL - 5} y={yFor(t) + 3} textAnchor="end" fontSize="8" fill="#64748b">{t.toFixed(1)}</text>
              </g>
            ))}
            ...
            <text x={padL} y={H - 7} textAnchor="start" fontSize="8" fill="#64748b">{dates[validPoints[0].i]}</text>
            <text x={W - padR} y={H - 7} textAnchor="end" fontSize="8" fill="#64748b">{dates[validPoints[validPoints.length - 1].i]}</text>
          </svg>
    ...
    const MiniWeightChart = ({ title = 'Динамика веса', data, dates, color = '#8c78c6', unit = 'кг' }) => {
      const points = (data || [])
      ...
      return (
        <div className="card-enter bg-zinc-900/40 rounded-2xl border border-zinc-800/40 p-3 mb-4">
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto block">
            {yTicks.map((tick) => (
              <g key={tick}>
                <line x1={padL} y1={yFor(tick)} x2={W - padR} y2={yFor(tick)} stroke="rgba(142, 167, 195, 0.16)" strokeWidth="1" />
                <text x={padL - 5} y={yFor(tick) + 3} textAnchor="end" fontSize="8" fill="#71717a">{tick}</text>
              </g>
            ))}
            <polyline points={line} fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
            {points.map((p, idx) => <circle key={idx} cx={xFor(p.i)} cy={yFor(p.v)} r="2.2" fill="#aeb8ff" />)}
            <text x={padL} y={H - 2} fontSize="9" fill="#71717a">{dates[points[0].i]}</text>
            <text x={W - padR} y={H - 2} textAnchor="end" fontSize="9" fill="#71717a">{dates[points[points.length - 1].i]}</text>
          </svg>
        </div>
      );
    };
