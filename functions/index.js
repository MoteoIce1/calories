...
// Для распознавания блюд ПО ФОТО нужна более сильная зрительная модель — Haiku часто
// путает блюда и выдумывает ингредиенты. Sonnet 4.6 заметно точнее в vision; дороже,
// но фото — редкое действие и под дневным лимитом. (Для максимума можно 'claude-opus-4-8'.)
...
exports.analyzePhoto = onCall({ secrets: [ANTHROPIC_API_KEY], region: 'us-central1', enforceAppCheck: true, memory: '512MiB' }, async (request) => {
  ...
  const system = 'Ты нутрициолог-эксперт по распознаванию еды на фото. Внимательно рассмотри изображение и определи блюдо ПО ТОМУ, ЧТО РЕАЛЬНО ВИДНО, не угадывай наугад. '
    ...
    + 'grams — примерный вес порции в граммах; calories/protein/fats/carbs — КБЖУ ВСЕЙ порции (ккал и граммы, не на 100 г). '
